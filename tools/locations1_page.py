...
SECTIONS = [
    ('monorail', 'Monorail (inside the carriage)', 'game/img/bg/monorail.webp',
     'The game now uses the outside view, which runs on ordinary rails on a bridge. These are inside the carriage, with the bay, the beam and the island through the windows.'),
    ('gate', 'Gate (security entrance)', 'game/img/bg/gate.webp',
     'The current one looks like a station ticket hall with no guard post. These have ID card gates and a place for Ishibashi to stand.'),
    ('copyroom', 'Copy room', 'game/img/bg/copyroom.webp',
     'Where he first uses magic on the 30 copies, so it should feel shut away: no windows, one door with a small window onto the corridor, an old bulky copier in the middle of things.'),
    ('sales', 'Sales (third floor)', 'game/img/bg/sales.webp',
     'The current one has no monitors and a glass ceiling. These are busy desk rows with screens, phones and a target board, to contrast with the basement.'),
]
...
open(os.path.join(OUTD, 'index.html'), 'w').write('''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Day-1 locations, round 1</title>
<style>
...
<h1>Day-1 locations, round 1</h1>
<p class="intro">New backgrounds for the monorail, the gate, the copy room and Sales, drawn with RDBT Anima at the game's shape. The basement office is in the cast round (decide2). Each set starts with the current game image in grey. Nothing is in the game yet. Click an image to enlarge; arrow keys step through, Esc closes.</p>
