...
- Day-1 flow (2026-09-25): monorail → gate → basement office → copy room → office → Sales. Five drawn locations.
...
- Prompt only what the camera sees (images and video). Work out the camera position and direction first, then describe only what is in that frame. A shot of an oncoming train looks back along the track, so the island is behind the camera and must not be mentioned; naming it pulls it into the frame and breaks the logic. Give trains a visible end (a nose and a last car, a set number of cars) and a clear direction of travel that matches the story.
